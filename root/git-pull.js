let options
const argsSchema = [
  ['github', 'alexrusselldev'],
  ['repository', 'bitburner-home'],
  ['branch', 'main'],
  ['download', ['root']], // By default, all supported files in the repository will be downloaded. Override with just a subset of files here
...
async function repositoryListing(ns, folder = '') {
  ...
  try {
    ...
    for (const folder of folders)
      files = files.concat(
        (await repositoryListing(ns, folder)).map((f) => `/${f}`)
      ) // Game requires folders to have a leading slash
    return files
  } catch (error) {
    if (folder !== '') throw error // Propagate the error if this was a recursive call.
    ns.tprint(
      `WARNING: Failed to get a repository listing (GitHub API request limit of 60 reached?): ${listUrl}` +
        `\nResponse Contents (if available): ${JSON.stringify(
          response ?? '(N/A)'
        )}\nError: ${String(error)}`
    )
